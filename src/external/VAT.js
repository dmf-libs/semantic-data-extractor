...
export default function checkVATNumber(toCheck) {
...
function ROVATCheckDigit(vatnumber) {
  ...
  // Compare it with the last character of the VAT number. If it's the same, then it's valid.
  if (total == vatnumber.slice(vatnumber.length - 1, vatnumber.length)) return true;
  else return false;
}
...
function RUVATCheckDigit(vatnumber) {
  ...
  if (vatnumber.length == 10) {
    ...
    if (total1 == vatnumber.slice(10, 11) && total2 == vatnumber.slice(11, 12)) return true;
    else return false;
  }
}
